    ...
    <div className="flex flex-col h-dvh overflow-hidden bg-background text-foreground pt-[env(safe-area-inset-top)]">
      ...
      <header className="shrink-0 flex items-center justify-between gap-4 border-b border-border px-4 h-11">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            title="Søg (⌘K)"
            className="flex items-center gap-2 rounded-md border border-border bg-muted px-2 py-1 min-h-8 text-xs text-muted-foreground transition-colors hover:text-foreground hover:bg-muted/80"
          >
            <Search size={13} />
      ...
      <main className="flex flex-1 min-h-0 overflow-hidden">
        <div className={cn('flex-1 min-w-0 relative flex-col', mobileTab === 'map' ? 'flex' : 'hidden', 'lg:flex')}>
          ...
          {/* Chrome-less horizontal scroll: at 360px the five layer buttons +
              style toggle don't all fit at touch size. */}
          <div className="lg:hidden shrink-0 flex items-center justify-between gap-2 px-3 py-2 border-b border-border bg-background/90 backdrop-blur-sm overflow-x-auto no-scrollbar">
            <div className="shrink-0"><LayerControls activeLayers={activeLayers} onToggle={handleLayerToggle} /></div>
            <div className="shrink-0"><MapStyleToggle value={mapStyle} onChange={setMapStyle} /></div>
          </div>
          <DenmarkMap
            ref={mapHandle}
